...
**All scripts must be committed to git.** Historical lesson: an earlier
autonomous prompt did `git stash -u` and *vanished* the untracked scripts
mid-run — every subsequent `at` job fired into a missing file. The current
prompt no longer stashes (it auto-commits pending WIP at step 1 instead), but
treat `.claude/scripts/` as production infra anyway.

## Derived variables (in every script)

All scripts derive their context **from their own location** — they live at
`<repo>/.claude/scripts/`, so the project root is two levels up. **No
project-specific constants to edit.**

```bash
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"            # absolute repo root
...
.claude/scripts/view-task-history.sh --list

# Pretty-print by session id
.claude/scripts/view-task-history.sh <session-id>

# Or by (partial) task name — picks most recent match
.claude/scripts/view-task-history.sh <task-name-fragment>
...
5. **+3 min gap.** Gives the previous run's post-exit usage-stats commit time to land before the next run's auto-commit step picks up its leftover.
...
- **Self-chain runaway** — a chain only advances on `ok`; the agent arms exactly one `at` job (+3 min) for the single next card, so at most one chained job is ever queued. `fail`/`skip` (or dirty tree at step 9) breaks the chain rather than looping.
...
- **Scripts derive paths from `$BASH_SOURCE`** — no env vars or constants to edit per repo.
...
Move the `.claude/scripts/*.sh` + `.claude/skills/schedule-tasks/` folders
into a new repo and it works as-is, provided `.claude/kanban/todo/` exists and
`atd` is running.
