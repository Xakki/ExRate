...
the agent self-chains the next card at **+3 min**. On `fail` / `skip` /
empty backlog the chain **STOPS** — no further jobs queued.

This skill is **project-agnostic**: scripts derive paths from their own
location, the prompt discovers subagents/test-commands from the host project's
`CLAUDE.md`. Drop the `.claude/scripts/*.sh` + `.claude/skills/schedule-tasks/`
folders into any repo that uses the `.claude/kanban/{todo,progress,test,ready,done}/`
layout and it works as-is (`ready/` is auto-created on first transition).
...
test -x .claude/scripts/run-claude-task.sh           # outer script present + executable
test -x .claude/scripts/run-claude-task-inner.sh     # inner script present + executable
git ls-files --error-unmatch .claude/scripts/run-claude-task.sh .claude/scripts/run-claude-task-inner.sh
                                                     # scripts MUST be tracked — see reference.md
```
...
RUNNER="$REPO/.claude/scripts/run-claude-task.sh"
...
3. **Step 9 — chain the next card.** On a green outcome (card landed in
   ...
   (related-first → lex-oldest) and runs `at -t +3min` to enqueue exactly
...
**Kill an in-flight chain:** `atq` → `atrm <N>` for the queued +3 min job (at
