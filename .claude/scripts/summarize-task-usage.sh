...
#
# Universal: derives PROJECT_DIR / CLAUDE_PROJECT_PATH from its own location
# (script lives at <repo>/.claude/scripts/summarize-task-usage.sh).
#
# Usage: .claude/scripts/summarize-task-usage.sh <session-id> <task-file> [<wall-seconds>] [<exit-code>] [<result>]
set -uo pipefail
...
block.append("")
block.append(f"_Prices approximate — see `.claude/scripts/summarize-task-usage.sh` PRICE table._")
block.append("")
